...
- Aggressive `VACUUM` and index maintenance on `call_queue` and `call_attempts`; partition maintenance for `audit_logs` (see below); other operational tables stay flat until Phase B.
