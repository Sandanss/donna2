...
3. **On decline:** flip a new dedicated `seniors.callable = false` column and enqueue a caregiver notification. Rationale: `isActive` is caregiver-controlled soft-pause; `callable` is consent-driven. Keeping them separate preserves the ability to distinguish "caregiver paused" from "senior declined". Scheduler/queue queries gate on `callable = true AND consent_status = 'granted'` — but only when `SCHEDULER_REQUIRE_CONSENT=true` (see `lib/scheduler-config.js#isSchedulerConsentGateEnabled`). Default OFF so the merge to prod doesn't lock out existing seniors if migration 014's grandfather UPDATE didn't run for some reason. Flip the flag on prod after verifying with `SELECT consent_status, callable, COUNT(*) FROM seniors GROUP BY 1, 2;`. Migration ships the partial index `idx_seniors_dispatchable` covering the triple-predicate.
