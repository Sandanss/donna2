...
| P0 | Prove inbound known-senior and onboarding voice calls. | `pipecat/api/routes/telnyx.py`, `pipecat/bot.py`, `pipecat/flows/nodes.py`, `pipecat/services/prospects.py` | Known senior inbound calls load profile context; unknown callers use onboarding flow and prospect handling without senior reminders. | Dev Telnyx inbound tests for one known test number and one unrecognized test number |
