...
| **Google (Gemini)** | Conversation transcripts for Director fallback; new customer summary/evaluation payloads where enabled | **Yes** | NOT SIGNED | **Yes** | Google Cloud with BAA enabled | Gemini Flash is used for Director fallback and lightweight new-customer/evaluation paths, not the active subscriber post-call analysis path. Enable BAA in Google Cloud Console under "Healthcare" settings. |
