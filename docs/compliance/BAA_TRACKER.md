...
| Service | Data Processed | BAA Required? | BAA Status | BAA Available? | Plan Required | Notes |
|---------|---------------|---------------|------------|----------------|---------------|-------|
| **Anthropic (Claude)** | Full conversation transcripts (system prompt + all user/assistant turns), user-entered reminders, health discussions if disclosed by users, senior names, profile context, memory context | **Yes** | NOT SIGNED | **Yes** | Enterprise | Claude Haiku 4.5 is the primary conversational LLM. All conversation content and prompt profile context pass through Anthropic. Contact sales@anthropic.com for enterprise BAA. |
| **Google (Gemini)** | Conversation transcripts (Director fallback), post-call analysis transcripts, call quality assessments, caregiver-facing summaries/takeaways | **Yes** | NOT SIGNED | **Yes** | Google Cloud with BAA enabled | Gemini Flash used for: (1) Director fallback analysis, (2) Post-call analysis + call quality scoring, (3) caregiver-facing summary/takeaway generation. Enable BAA in Google Cloud Console under "Healthcare" settings. |
