...
'
  ...
  case "$status" in
    ...
    FAILED | CRASHED | REMOVED)
      ...
      echo "Railway build/deployment logs are intentionally not copied into CI output."
      echo "Inspect deployment ${deployment_id} in Railway with production log access controls."
      exit 1
      ;;
  esac
