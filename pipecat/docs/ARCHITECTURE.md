...
- Memory context, recent turns, and same-day context
...
**Guidance handoff**: When patterns match, Quick Observer builds a guidance string (for example, `[EMOTION] They seem upset. Acknowledge their feelings.`) and stores it in `session_state["_pending_observer_guidance"]`. Conversation Director then injects it as an `LLMMessagesAppendFrame` with `run_llm=False`. This single-writer handoff keeps Observer guidance current-turn without duplicate ephemeral messages.
...
TELNYX_ANSWERING_MACHINE_DETECTION=premium
TELNYX_VOICEMAIL_MESSAGE="Hi, it's Donna. I was calling to check in. I'll try again later. Take care."
TELNYX_VOICEMAIL_VOICE=female
TELNYX_VOICEMAIL_FALLBACK_DELAY_SECONDS=8
