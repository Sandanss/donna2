...
- The mapping is per-person — verify with the local branch list rather than guessing a transform.
...
# stage as above
git commit -m "<type>: <specific, descriptive subject>"
```
...
If the push is rejected because remote `<name>dev` has diverged, do **not** force-push without asking — someone else may have pushed work to it. Investigate first.
...
```bash
gh pr create --base main --head <name>dev \
...
- **Existing topic branch already open as a PR** — keep using it; don't migrate mid-flight.
